...
Loop
{
    ...
    mainCount++
    IniRead, %deref1%HotKey, %scriptName%.ini, main, value%A_Index%, %A_Space%
    %deref1%Count := 0
    Loop
    {
        IniRead, %deref1%Key%A_Index%, %scriptName%.ini, %deref1%, key%A_Index%, %A_Space%
        IniRead, %deref1%Value%A_Index%, %scriptName%.ini, %deref1%, value%A_Index%, %A_Space%
        If (%deref1%Key%A_Index% == "" And %deref1%Value%A_Value% == "")
        {
            Break
        }
        %deref1%Count++
        Hotkey, % "#" . %deref1%HotKey, CUSTOM
    }
}
Return

CUSTOM:
StringRight, letterKey, A_ThisHotkey, 1
Loop % mainCount
{
    deref1 := main%A_Index%
    If (letterKey == %deref1%HotKey)
    {
        Break
    }
}
